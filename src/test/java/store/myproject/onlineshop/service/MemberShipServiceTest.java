...
class MemberShipServiceTest {
    ...
    void get_membership_by_id_success() {
        given(memberShipRepository.findByUuid(memberShip.getUuid())).willReturn(Optional.of(memberShip));

        MemberShipDto response = memberShipService.getMemberShip(memberShip.getUuid());
    ...
    void get_membership_by_id_fail_not_found() {
        given(memberShipRepository.findByUuid(memberShip.getUuid())).willReturn(Optional.empty());

        assertThatThrownBy(() -> memberShipService.getMemberShip(memberShip.getUuid()))
    ...
    void update_membership_success() {
        ...
        MessageResponse response = memberShipService.updateMemberShip(memberShip.getUuid(), request);
    ...
    void update_membership_fail_not_found() {
        ...
        given(memberShipRepository.findByUuid(any(UUID.class))).willReturn(Optional.empty());

        // when & then
        assertThatThrownBy(() -> memberShipService.updateMemberShip(memberShip.getUuid(), request))
                .isInstanceOf(AppException.class)
    ...
    void delete_success() {
        ...
        given(memberShipRepository.findByUuid(entity.getUuid())).willReturn(Optional.of(entity));
        given(messageUtil.get(MessageCode.MEMBERSHIP_DELETED)).willReturn("해당 멤버십이 삭제되었습니다.");

        // when
        MessageResponse response = memberShipService.deleteMemberShip(entity.getUuid());
    ...
    void delete_fail_not_found() {
        ...
        // when & then
        assertThatThrownBy(() -> memberShipService.deleteMemberShip(memberShip.getUuid()))
                .isInstanceOf(AppException.class)
                .hasMessageContaining(ErrorCode.MEMBERSHIP_NOT_FOUND.getMessage());
    }
