...
class ItemControllerTest {
    ...
    void create_item_success() throws Exception {
        ...
        given(itemService.saveItem(any(), ArgumentMatchers.<List<MultipartFile>>any()))
                .willReturn(response);
    ...
    void find_item_success() throws Exception {
        ...
        given(itemService.selectOne(1L)).willReturn(itemDto);
    ...
    void changeItem_success() throws Exception {
        ...
        mockMvc.perform(builder)
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.resultCode").value(SUCCESS))
                .andExpect(jsonPath("$.result.msg").value("수정 성공"))
                .andDo(print());
    }
